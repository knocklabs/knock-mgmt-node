...
describe('resource channelGroups', () => {
  ...
  test.skip('upsert: only required params', async () => {
    const responsePromise = client.channelGroups.upsert('channel_group_key', {
      channel_group: { channel_type: 'push', name: 'Push Notification Group' },
    });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
  ...
  test.skip('upsert: required and optional params', async () => {
    const response = await client.channelGroups.upsert('channel_group_key', {
      channel_group: {
        channel_type: 'push',
