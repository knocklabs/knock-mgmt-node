...
import { APIPromise } from '@knocklabs/mgmt/core/api-promise';
...
describe('logging', () => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new KnockMgmt({ logger: logger, serviceToken: 'My Service Token' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'KNOCK_MGMT_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new KnockMgmt({ baseURL: 'http://localhost:5000/custom/path/', serviceToken: 'My Service Token' });
    ...
    test('in request options overridden by client options', () => {
      const client = new KnockMgmt({ serviceToken: 'My Service Token', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['KNOCK_MGMT_BASE_URL'] = 'http://localhost:5000/env';
      const client = new KnockMgmt({ serviceToken: 'My Service Token' });
